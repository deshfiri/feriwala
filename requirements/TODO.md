...
# Pending frontend dependencies

Full-stack slices are the working rule: a business feature is not done until the people who operate
it can operate it. Where a screen genuinely cannot be finished because the module it needs has not
...
- [x] **P1-14** Login attempt limits + brute-force protection. Two limits, because the same attacker looks like two things: five failures a minute against one identity, and thirty in fifteen minutes from one address — the second is what a password sprayed across a thousand accounts trips, since each account sees a single failure. Deliberately generous on the address, because a whole office or a carrier NAT arrives here as one. `fortify.limiters.login` is **null** on purpose: naming a limiter puts the `throttle` middleware in front of the route, and middleware counts _requests_, so a successful sign-in spent from the same budget as a guess. Success clears the identity and **not** the address — an attacker working a credential list will eventually land on a password that works, and clearing there would hand them a fresh budget for the rest. Keys are hashed, so a Redis listing does not enumerate the addresses being tried, and the counters are pinned to Redis rather than the local node. 429 with `Retry-After` for an API client; the message under the email field for a browser, in English and Bangla — `44babf8`, 12 tests
- [x] **P1-15** Suspicious login detection + security notification. `authenticated_sessions` is the durable record beside Redis — sessions vanish when they expire, which is right for a session and useless as a history — and carries three jobs: the live sessions, the finished ones, and the devices already seen. Recorded by middleware, not a listener on `Login`, because Fortify regenerates the session identifier _after_ that event fires and every row would be unmatchable. The identifier is stored encrypted and found by a hash. **The first session a person is ever seen with is never new**, or the day this shipped would have alerted every existing user at once; novelty is judged per person, and a new address on a known device is deliberately silent, because people travel and alerting on that trains the reader to ignore the alert that matters. Mail as well as the bell: an attacker already signed in can read the bell. Geo is not implemented — it needs an IP-to-location provider, which is a dependency decision — `a697767`, 11 tests
- [x] **P1-16** Device & session history; "sign out other devices". The list is the security control, not decoration. **Ending a session destroys it** — the row is bookkeeping and the handler reaches into Redis first, because a screen that says a device was signed out while it carries on working is the worst outcome for the one control people reach for when they think they are compromised. Scoped by the relation rather than by a check afterwards, so §31.3 holds by construction and a stranger's public id is a 404. The identifier never reaches the browser. Ending the session doing the asking is refused: that is what signing out is. Behind password confirmation, because somebody at an unattended laptop must not be able to sign the owner out of everywhere else and keep the seat they are in — `3b58afe`, 10 tests
- [x] **P1-17** Account lock/unlock with reason + audit. The **identity**, not the business: a locked person loses every screen including administration, and their business account stays exactly where it was (D23). A reason is required in both directions and never reaches the person (§7.3). Locking ends the sessions, because a lock that leaves an open tab working until the next reload is not a lock. Two rules sit above the permission, in two places on purpose — _only a Super Admin locks a Super Admin_ in the policy, since `Gate::before` answers first for the case being allowed; _nobody locks themselves_ in the action, since `Gate::before` would pass straight over a policy rule. Reached from a "Sign-in access" panel on the account dossier, with `can_change` asked per person so no control is offered that the action will decline — `bac8873`, 13 tests
- [x] **P1-18** 2FA and passkeys verified against §6; a second factor required before the admin panel (§36). **Which roles is derived, not listed**: `PlatformRole::requiresTwoFactor()` reads §32.2's own sensitive-action set off the permissions a role actually holds, so a role gaining `adjust_wallet` gains the requirement in the same edit — a hand-kept second list drifts, and drifts silently. Read from `permissions()` rather than the raw grants, because the catalogue drops verbs a module does not accept. A Report Viewer stays outside, so the boundary is real. The answer is a redirect to enrolment, not a 403: somebody newly given a role has one step to take, not a missing account. An unconfirmed secret does not count as enabled. The Fortify configuration §6 depends on — confirm-before-trust and password confirmation for both two-factor and passkey enrolment — is now asserted rather than assumed — `42c9a0d`, 12 tests
- [x] **P1-19** Password reset security review. A reset is what somebody does after losing control of the account, and on its own it did nothing to a session already open — so the reset felt like a fix while changing nothing. Every session now ends on `PasswordReset`; a change from the security screen ends the others and keeps the seat. There was **no notification at all** for a password change from either path; there is now, by mail above all, because an attacker who has just taken the account can read the dashboard and not the inbox. The reset form was an **enumeration oracle** — Fortify reports the broker's status on the email field, so "no such user" and "check your inbox" answered whether somebody banks here, with no credentials needed; both failure statuses now answer identically to success, throttling included, since only a real address can be throttled. Token expiry and the resend interval are named with reasons instead of sitting at framework defaults, and single use is asserted rather than assumed — `bf08ff7`, 10 tests

## P1.C KYC (§7) — complete

All of §7 is built: dynamic requirements with package and country scoping, per-round requirement
snapshots, document security, review, deadlines, administrator-requested updates, and the
applicant's own history.

- [x] **P1-20** `ManageKycDocumentTypes` + `admin/kyc/document-types` — create, edit, pause, resume, reorder, archive, delete. **Three** states, not two: active is on the form, paused is off but can return, archived is retired for good and cannot be edited, because it is part of the record of what past rounds asked for. A type any submission has referenced is **never deleted** — the action refuses it even for Super Admin, whose `Gate::before` passes every policy, so the rule lives where a grant cannot override it. Permissioned on `kyc.manage_settings`: shaping the catalogue decides what every future applicant provides, reviewing decides one case
- [x] **P1-21** Scope rules carry a nullable **package and country**, so "Enterprise accounts in Bangladesh" is expressible; a scope may also override required/optional without a duplicate document type. Documented priority, most specific first: package+country → country → package → global, with **country outranking package** because a country rule is usually statutory and a commercial plan must not waive a regulator. A type with no scopes is global — the safe fallback. **Neither dimension is free text**: a package slug must resolve to a live package and a country must be one `config/countries.php` lists, so a rule cannot be saved that looks configured and matches nobody. Where no packages exist yet the control is absent and says why (FD-2) rather than offering a box to guess into. `kyc_submission_requirements` **snapshots** what each round was opened against, so editing a type cannot rewrite what an in-progress or historical round was judged on; submission, upload validation and the applicant form all read the snapshot. A resubmission is snapshotted afresh, deliberately: copying stale rules forward would keep demanding a withdrawn document — 22 tests
...
- [x] **P1-27** `ApplicantKycHistory` + `/kyc/history` — round status, requested document types and whether each was provided, dates, deadline and days left, outcome, and the feedback written **to** the applicant. What it omits is the point: the reviewer's internal note, the internal reason behind a requested update, the reviewer's identity, and every file path, disk and checksum (§7.5). Assembled in one place so there is one thing to audit and one thing to test; asserted on the response as well as the payload, because a payload is only safe by the time it reaches the browser. Self-scoped from membership — no account identifier in the URL (§31.3) — 20 tests
...
- [x] **P1-65** `{current_team}` gone from every URL, along with the switcher and the whole team concept — tables, models, policies, routes, labels. Account context comes from the person's one membership, never from the URL or a session value, so there is no identifier to swap for somebody else's (§31.3). Shared to Inertia as a single `account` prop rather than a `currentTeam` + `teams` pair, because shipping a list is what makes a front end build a switcher for it
- [x] **P1-66** `AccountInvitation` — single-use, expiring, revocable, unique per open address by a **partial index**, and matched on the invitee's email plus verified mobile when one was given. Staff sign in with their own credentials and hold their own `AccountRole`. Registering with an invitation opens **no** business: without that the invitee would leave registration owning an account, that account would take the one membership D1 allows them, and the invitation would be refused for a conflict registration had just created
- [x] **P1-67** `StaffAllowance` — the owner consumes no seat (a limit of one means one staff member, not "the owner and nobody"), and a **live invitation holds one** until it is used, withdrawn or expires, so ten invitations cannot be sent against two seats. Checked inside a transaction that locks the account, and re-checked at acceptance because a package can shrink in between; an invitation is permission to ask, not a reservation that outranks the package
- [x] **P1-68** A package with no staff facility has no Staff area at all — absent, not disabled, and the endpoints refuse as well as the buttons being hidden. The owner is unremovable and undemotable through staff management whatever the caller is permitted to do, including Super Admin, because that is an invariant of the account rather than a grant. Vocabulary is Account, Staff, Permissions; a test asserts no route or route name contains "team" — 47 tests
- [x] **P1-69** `TaxEngine` — the single place tax is worked out. Rates are held in **basis points** (1500 is 15.00%), never a float: 7.5% as a float is 0.07500000000000001, and that lands in a ledger that must reconcile to the poisha. Exclusive is the default; inclusive extracts by rounding the net and taking the tax as the **remainder**, so net + tax always equals the price the customer was shown. With no rule configured the answer is **no tax** — D19 forbids assuming a statutory rate, so an unconfigured system undercharges visibly rather than inventing 15%
- [x] **P1-70** `tax_rules` scoped to product, category, fee or everything, resolved **most specific first**. Priority breaks ties within a level and never across one, so a high-priority catch-all cannot silently override every targeted rule beneath it. Rates are **versioned rather than edited** — a rule names a rate by code, and the code resolves to whichever version was in force when the charge was made, so a reissued invoice still agrees with itself
- [x] **P1-71** `tax_exemptions` — a dated grant with a reason, a certificate reference and who gave it, not a boolean on the account. An expired certificate stops being honoured on its own, and "why was no VAT charged in March" has an answer years later. Revoked, never deleted
- [x] **P1-72** `payment_tax_lines` — the tax charged per rate, beside the single rolled-up Tax allocation. The code, label and basis points are **copied in**, so a rate change next year cannot rewrite an issued invoice; the taxable base is stored beside the tax because a return needs both and dividing one back out goes wrong on the rounding. The discount is apportioned across fees so each is taxed at its own rate — 47 tests
- [~] **P1-73** `RefundabilityPolicy` + `RefundStatus` + `RequestRefund` / `DecideRefund`. Three rules, not a boolean — never, before activation, always — because "refundable" cannot express the package fee's actual condition. Defaults are the **restrictive** reading, so a misconfiguration refuses a refund a human can grant rather than paying out money nobody agreed to; the wallet deposit is the exception, since it was never Feriwala's. Eligibility is re-checked **at approval**, because an account activated between request and decision changes the answer. `Approved` and `Processed` are separate states so a failed gateway refund cannot look like money that went back; rejections are kept with their reason — 25 tests. _Ledger posting attaches when the ledger lands (Phase 2); an approval moves no money on its own, so nothing goes unrecorded meanwhile_
- [x] **P1-74** `DowngradeGuard` reports **every** exceeded limit at once with the three figures D16 requires — what you have, what the package allows, how many must go — rather than a bare refusal, and rather than one at a time so a downgrade does not take four attempts. Counts are passed in, as with `Entitlements::hasCapacityFor`, so the caller counts the thing the limit actually governs. Quotas that reset with the term are not guarded. `OverrideDowngradeLimit` needs the permission **and** a recorded reason, writes a sensitive audit entry naming exactly what was waved through, and **removes nothing** — an override that deleted the excess would be the auto-removal D16 forbids — 14 tests
...
- [x] **P1-38** Package renewal (§8.2, §8.4). A renewal is a **new subscription record**, never the old row given more time: every term an account has held is history — what it cost, what it granted, when it ran — and extending in place overwrites all three. `renews_user_package_id` links the chain rather than leaving it to be guessed from dates. Terms are captured **fresh** at the moment the renewal is opened, which is the point §8.3 defines for a change of terms: this year's price and entitlements, shown in full before anything is paid, while the term running out keeps its own snapshot untouched. The renewal fee where the package sets one and the package fee where it does not — §8.1 makes it optional, and silence there means no separate renewal price rather than a free year. No registration fee and no deposit: one is charged once at §9, the other is already the partner's money. **The new term starts where the old one ends**, so renewing a month early does not throw that month away and paying on time is not a penalty; a term already run out starts now and is closed. Nothing is granted until the money clears — the renewal is created awaiting payment, `SettlePayment` activates it on §8.4's "successful renewal verification", and a repeated gateway callback finds it already live and does nothing. Renewal stays reachable **after** expiry, because §8.4 keeps payment and renewal open and a page that closes at expiry closes exactly when it is needed — `a4be623`, 25 tests
- [x] **P1-39** Package upgrade and downgrade (§8.3). The two directions are **deliberately not symmetrical**, because the risk is not: an upgrade applies at once, since somebody paying for more capacity needs it now, and a downgrade applies at the end of the term the account has **already paid for** — taking capacity away mid-term would remove something already bought, and it gives whoever is over a limit the rest of the term to get under it (D16). An upgrade keeps the billing cycle: the new term ends when the old one would have, so a change of package does not quietly become a change of renewal date. Proration credits the unused share of what was **actually paid**, in integer minor units floored so the credit is never more than the account is owed, and the credit is a discount **line** so tax is charged on what is really being bought rather than on days nobody is buying. It can never become a payout — refunds are their own workflow with their own approval (§27) — and that guard is asserted directly rather than left as a comment. Deposits move one way only: §8.3's additional requirement is the **difference**, and a smaller package does not release one here. `AccountHoldings` gives `DowngradeGuard` the counts it deliberately refuses to query for itself; products and websites read zero until their modules exist, which is the honest answer. Nothing moves in either direction until the money clears — an unpaid downgrade taking a limit away would be a restriction imposed for free — `ecd7f60`, 22 tests
- [x] **P1-40** Manual and promotional package assignment (§8.3). Real entitlement and **no money** — which is the whole difficulty, because revenue reports, renewal quotes and refund eligibility all have to tell it from a sale. So nothing invents a transaction that never happened: `paid_fee_minor` is zero, `SubscriptionSource::isPaid()` answers false for both sources, and a test asserts no payment row is created at all. **The dates are given, not derived**: §8.3 lists an effective date among what an administrator configures, and a promotion runs for its promotion rather than for whatever the package's validity says this month. A reason is required and goes to the audit trail, never to the account (§7.3) — somebody will ask why this business has a plan nobody paid for, and "an administrator assigned it" without a because is not an answer. The status still moves through the machine: created awaiting payment and transitioned to active in the same breath, which is the declared route rather than an assignment that skips the one guard the enum exists for. A grant starting now closes the term it replaces as cancelled; one dated ahead leaves the current term running. Behind `package.manage_settings` rather than `package.create`, because writing a plan and giving one away are different decisions — `7ca9524`, 15 tests
- [x] **P1-41** Cancellation, expiry and the grace period (§8.2, §8.4). A daily sweep makes three moves in order — a live term approaching its end becomes **renewal due**, one past its end with grace left enters the **grace period**, one past that has **expired** — and a term whose end went by while nothing was sweeping goes straight from active to expired, which the status map allows precisely so that gap needs no special case. **Idempotent by transition rather than by a marker table**: every move is made under a row lock and only from a state that permits it, so a second pass finds the term already moved and the notification beside it is sent exactly once however many servers run the sweep. `onOneServer` and `withoutOverlapping` on the isolated lock database, in the platform's timezone, because "expires on the 30th" has to mean the 30th where the account holder is. **What expiry means is not decided in the sweep**: `entitles()` already stops an expired term granting anything and every feature gate reads `Entitlements`, so §8.4's restriction happens by the term changing state rather than by a scheduled job reaching into other modules. Renewal and payment stay open afterwards (§8.4), and a paid renewal restores everything. Cancellation **stops the term now** and says so before the button — leaving access on while calling a term cancelled would mean the status no longer describes what the account can do — and money already paid is left to the refund workflow with its own approval (§27) rather than quietly returned. The pointer moves off a closed term onto a successor, or is cleared — `f9702b0`, 18 tests
- [x] **P1-42** Package invoice and payment history (§8.2, §9). An invoice is **not a view over a payment**: a payment can be retried, fail, settle late or be reconciled against a gateway reporting something else, and the document a customer already has must go on saying what it said. So the lines are **copied at issue**, labels included — a fee renamed next year must not retitle a line on somebody's filed invoice — and both tables refuse updates and deletes the way the audit log does. Issued when the payment is **recorded**, not when it settles: an invoice is what somebody is asked to pay, and waiting for the money would leave an account with a figure and nothing itemising it. That is not granting anything — whether the money arrived stays the payment's business, read through `isPaid()` every time rather than duplicated into a column that would drift, and nothing in the subscription lifecycle reads an invoice at all. Idempotent at the index: `payment_id` is unique, so two concurrent requests both try and the loser returns the winner's row. Numbers are year-scoped and derived from the row id, unique by construction with no counter to keep consistent. Reached through the account's own relation, so a stranger's identifier is a 404 rather than a refusal — `8bf7755`, 13 tests

## P1.E Combined registration + package payment (§9)

- [x] **P1-43** `fee_rules` — dated prices, not an editable setting (§9). The registration fee lived in a settings row: one value, no history, and rewriting it left nothing recording that it had been raised. **A price is never edited**: changing a fee opens a new rule and closes the one it replaces, so a quote resolved against a date reproduces itself and "what was the fee in March" has an answer. Resolution walks outward from the most specific rule — a package-scoped rule, then the package's own column, then the global rule — because **specificity beats recency** and a newer global price must not quietly override a deal struck for one plan. `$at` is the moment the charge is made, not the moment the code runs. Nothing configured means **zero**, not a guess, the same reasoning D19 applies to tax. Overlapping windows at one level are refused rather than resolved, and closing a rule dates it rather than deleting it — a rule a payment was priced against is part of that payment's story. Behind `payment.manage_settings`, deliberately not `package.edit`: writing a plan and pricing the fee charged alongside it are different jobs — `cec2beb`, 17 tests
...
- [x] **P1-46** Coupons and promotional discounts with effective dates (§9). The whole design turns on one distinction: a code is **held**, not spent, when somebody commits to paying. Checking what a code is worth runs on every checkout render, so a coupon consumed by validation would be consumed by anyone who opened the page — `CouponValidator` reads and nothing else, and `ReserveCoupon` runs once, at the moment a payment is recorded. A reservation counts against the limits while it is held, becomes a redemption when the money arrives, and goes back if it never does; a released row is kept with its timestamp rather than deleted, because "tried in March and did not pay" is worth being able to see. The coupon row is **locked before the limits are counted** — reading the count first and locking after is the classic way to oversell the last slot — and `payment_id` is unique, so a double submit settles at the index rather than at a check the retry would also pass. Every refusal names its own cause: expired, not yet started, wrong package, minimum spend, exhausted, already used are six different problems with six different next steps, and "not valid" answers none of them. Percentages are basis points floored in minor units, capped, and never larger than the base — a coupon cannot turn a sale into a payout. Scope is per fee (§9 stores the registration fee and the package fee separately), so "20% off the package fee" cannot quietly discount the other. Withdrawing closes a coupon rather than deleting it: an invoice names the discount it produced — `0181eca`, 30 tests
- [x] **P1-47** Tax rules applied at checkout, with the settings screen behind them (§9, D19). The engine already resolved rules and rates; what was missing was everything around it — nobody could configure a rate without writing a row by hand, and the checkout showed one rolled-up "VAT" line that cannot answer "at what rate, on what", which is the only question somebody filing a return has. Rates and rules are two panels because they are two decisions: a rate is what a code is worth on a date, a rule is what that code applies to, and merging them would make "VAT moved to 12%" look like an edit to every rule naming it. Both are opened and closed, never edited. The guard the task turns on is that a rule must never be **silently** anything: a rule naming a code that does not exist is refused at the form, and a live rule whose rate has since been withdrawn charges nothing — D19 forbids inventing a statutory rate — but says so, in the log and on the row, because a rule quietly taxing nothing is indistinguishable from one that works until a customer notices. Overlap is refused at both levels: two rates open under one code, or two rules of identical scope, target and priority, would make what is charged depend on row order — `b6417f6`, 36 tests
- [x] **P1-48** Payment deadline configuration and expiry (§9). §9 lists "Payment deadline" among the things an administrator configures and leaves the number to them, so it is **opt-in**: with `billing.payment_deadline_hours` unset nothing expires, which is the right state for an installation where nobody has decided yet. The deadline is **stamped onto the payment** rather than derived on every read — an applicant told they have until Friday still has until Friday after somebody shortens the window on Wednesday. Expiry cancels through the existing state map, and that is what makes "never expires a settled payment" structural rather than a query somebody has to remember to write: `Paid` has no move to `Cancelled` at all. `Pending` is deliberately excluded too — that is money in flight, a bank transfer or a risk review a gateway will still confirm, and cancelling it would abandon a payment on its way. Idempotent by transition under a row lock, so a second pass finds it already cancelled; hourly rather than daily, because a deadline configured in hours must not mean anything up to a day more; `onOneServer` and `withoutOverlapping` because it gives coupon slots back and two servers releasing one hold would hand out a promotion the limit did not allow. Expiry releases exactly what that checkout held — the reservation keyed to **that payment id**, never another account's — and releases its idempotency key with it, so the applicant's next attempt is a new payment at the current price rather than a resurrected row. The package selection is left alone: missing a deadline should cost somebody their checkout, not their place in the funnel. A failed payment now releases its coupon too, which it did not before — the slot was held for ever on an attempt that can never be revived — `a71cd05`, 24 tests
